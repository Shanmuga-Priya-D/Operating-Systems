...
int main()
{
	...
	while(i<n)
	{
		...
		p[pid].wt=p[pid].ft-p[pid].st;
		cur_time+=p[pid].st;
		p[pid].status=1;
		i++;
	}
	cout<<"process "<<" Arrival Time "<<" Service Time "<<"  Finish Time  "<<" Waiting Time"<<endl;
	for(int i=0;i<n;i++)
	{
		cout<<i+1<<"\t  "<<p[i].at<<"\t\t"<<p[i].st<<"\t\t"<<p[i].ft<<"\t\t"<<p[i].wt<<endl;;
	}
}
